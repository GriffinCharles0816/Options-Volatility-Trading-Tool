...
class VolatilityCrushAnalyzer:
    ...
    def setup_market_data_section(self, parent, row):
        ...
        ttk.Entry(data_frame, textvariable=self.strike_price_var, width=15, font=('Arial', 10, 'bold')).grid(row=2, column=1, sticky=(tk.W, tk.E), pady=(0, 8))
    ...
    def seteup_scenario_section(self, parent, row):
    ...
    def setup_new_greeks_section(self, parent, row):
        ...
        ttk.Label(new_greeks_frame, text="Delta (δ):").grid(row=0, column=0, padx=(0, 10), pady=(0, 5), sticky=tk.W)
        ...
        ttk.Label(new_greeks_frame, text="Gamma (γ):").grid(row=0, column=2, padx=(0, 10), pady=(0, 5), sticky=tk.W)
        ...
        ttk.Label(new_greeks_frame, text="Vega (v):").grid(row=1, column=0, padx=(0, 10), pady=(0, 5), sticky=tk.W)
        ...
        ttk.Label(new_greeks_frame, text="Theta (θ):").grid(row=1, column=2, padx=(0, 10), pady=(0, 5), sticky=tk.W)
    ...
    def fetch_market_data(self):
        ...
        # Try to implement Live market data request 
    ...
    def price_current_straddle(self):
        ...
        # try implementing automatically updated as iv changes
        ...
        vega = self.calculate_vega(spot_price, strike_price, T, r, iv_decimal,) * 2 # multiply vega by 2 because we have twice the vega exposure for a call and a put
    ...
    def analyze_scenario(self):
        try:
            ...
            messagebox.showerror("Error", "Invalid spot price of IV values")
            return

        try:
            K = float(self.strike_price_var.get())
            days_to_expiry = int(self.days_var.get())
        except ValueError:
        ...
        self.new_straddle_label.config(text=f"S{new_straddle_price:.2f}", foreground='blue')
        ...
        new_vega = self.calculate_vega(new_spot, K, T, r, new_iv,) * 2 # multiply vega by 2 because we have twice the vega exposure for a call and a put
        ...
        self.new_vega_label.config(text=f"{new_vega:.3f}")
        self.new_theta_label.config(text=f"{new_theta:.3f}")

        self.update_status(f"Scenario complete: New price ${new_straddle_price:.2f}")


# partially differentiate blac_scholes with respect to spot price (S) = delta, spot price twice = gamma, time to maturity (T) = theta, volatility (sigma) = vega
